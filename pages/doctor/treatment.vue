<template>
  <div>
    <sysHead text="治疗卡" showBack></sysHead>
    <div class="head">
    ...
    <div style="overflow: hidden">
      <div class="main">
        ...
        <div class="card">
          ...
          <div class="row">
            <div class="label">治疗卡状态:</div>
            <div class="value">{{ selectList.IsClose ? '已结案' : '未结案' }}</div>
          </div>
          <div class="row">
          ...
          <div class="row">
            ...
            <div class="look" @click="todayMedicine">今日服药</div>
            <div class="look" @click="gotoSupplement">服药补录</div>
            <div class="look" @click="gotoDrugDrinkingRecord">查看</div>
          </div>
          <div class="row">
            <div class="label">不良反应记录:</div>
            <div class="look" @click="addAdverseReactions">记录不良反应</div>
          ...
          <div class="row">
            ...
            <div class="look" @click="fllowUpRegister" v-if="fomrData.CurrentDiagnosisId == selectList.DiagnosisId">
              去随访
            </div>
...
<script>
import { personsTreatmentCardInfo, pulmonarymedicinesMedicinesupplement } from "@/api/doctor.js"
export default {
  data () {
    return {
      activeStyle: {
  ...
  methods: {
    //获取患者信息
    async getInfo () {
      let res = await personsTreatmentCardInfo(this.personId)
      let Response = res.Response
      let arr = Response.TreatmentCardInfoDtos;
      let TreatmentCardInfoDtos = [];
      //为了把当前正在使用的治疗卡放最前面
      arr.forEach((i) => {
        if (!i.IsClose) {
          TreatmentCardInfoDtos.push(i)
        }
      })
      arr.forEach((i) => {
        if (i.IsClose) {
      ...
      arr = TreatmentCardInfoDtos
      arr.map((i) => i.name = i.TreatmentPlanName)
      this.list = arr;
      this.fomrData = Object.assign(this.fomrData, Response)
      this.formDate = this.$util.formatObj(this.fomrData)
      this.selectList = Object.assign(this.selectList, { ...arr[0], ...res.Response })
    },
    //今日服药
    async todayMedicine () {
      let res = await pulmonarymedicinesMedicinesupplement({
        diagnosisId: this.selectList.DiagnosisId,
        Arr: [this.$util.getTime(new Date(), 'yyyyMMddHHmm')]
      })
      uni.showToast({ title: '服药成功', icon: 'none', });
    },
    //记录不良反应
    addAdverseReactions () {
      this.$storage.setStorage("patientInfo", { ...this.selectList, isDoctor: true });
      uni.navigateTo({
    ...
    //去随访
    fllowUpRegister () {
      if (this.fomrData.IsVisited) {//IsVisited是否有随访记录
        uni.navigateTo({
          url: "/pages/doctor/moreFollowup?Name=" + this.selectList.Name +
            "&DiagnosisId=" + this.selectList.DiagnosisId +
            "&PersonId=" + this.selectList.PersonId +
            "&TreatmentPlan=" + this.selectList.TreatmentPlanName +
            "&FileNumber=" + this.selectList.FileNumber
        })
      } else {
        uni.navigateTo({
          url: "/pages/doctor/firstFollowup?Name=" + this.selectList.Name +
            "&DiagnosisId=" + this.selectList.DiagnosisId +
            "&PersonId=" + this.selectList.PersonId +
            "&TreatmentPlan=" + this.selectList.TreatmentPlanName +
            "&FileNumber=" + this.selectList.FileNumber
    ...
    gotoSupplement () {
      uni.navigateTo({
        url: "/pages/doctor/supplement?DiagnosisId=" + this.selectList.DiagnosisId
      })
    },
    //随访记录
    gotoFollowup () {
