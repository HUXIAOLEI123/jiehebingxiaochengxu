<template>
  <div>
    <div id="head">
      <sysHead text="检查记录" showBack slot="top"></sysHead>
      ...
      <div class="main">
        <div v-for="item of tableList" :key="item.InspectApplicationId" class="item">
          ...
          <div class="row">
            ...
            <div class="cRed" v-if="item.InspectResult == '病原学阳性'">{{ item.InspectResult }}</div>
            <div class="c15C489" v-if="item.InspectResult == '病原学阴性'">{{ item.InspectResult }}</div>
          </div>
          <div class="detail" v-if="item.FileRelationGuid" @click="lookDetail(item)">
  ...
  methods: {
    ...
    async getList () {
      var param = {
        ...
        ApplicationType: "1"// ApplicationType 2 复查 1 常规检查
      }
